...
// New utility function to check if auth is disabled
export const isAuthDisabled = async () => {
  try {
    // Try to sign in with a temporary email/password to see if auth is enabled
    const { error } = await supabase.auth.signInWithPassword({
      email: 'test@example.com',
      password: 'password123'
    });
    
    // If we get an error about email logins being disabled, auth is disabled
    if (error && (
      error.message.includes('Email logins are disabled') || 
      error.message.includes('Signups not allowed for this instance')
    )) {
      console.warn('Supabase auth is disabled:', error.message);
      return true;
    }
    
    return false;
  } catch (error) {
    console.error('Exception checking if auth is disabled:', error);
    return false;
  }
};
