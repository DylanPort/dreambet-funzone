  ...
  return <div className={`backdrop-blur-lg border rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg ${bet.status === 'open' ? 'bg-black/20 border-dream-accent1/30' : bet.status === 'matched' ? 'bg-black/30 border-purple-500/30' : bet.status === 'expired' ? 'bg-black/20 border-red-500/30' : bet.outcome === 'win' ? 'bg-black/20 border-green-500/30' : bet.outcome === 'loss' ? 'bg-black/20 border-red-500/30' : 'bg-black/20 border-yellow-500/30'}`}>
      <div className="px-6 py-4">
        ...
        {(bet.status !== 'open' || isExpired) && <div className="mt-4 space-y-3">
            ...
            <div className={`text-sm px-3 py-1.5 rounded-md text-center ${bet.status === 'matched' ? 'bg-purple-500/10 text-purple-400' : bet.outcome === 'win' ? 'bg-green-500/10 text-green-400' : bet.outcome === 'loss' ? 'bg-red-500/10 text-red-400' : isExpired ? 'bg-red-500/10 text-red-400' : 'bg-yellow-500/10 text-yellow-400'}`}>
              {bet.status === 'matched' ? 'Bet is active and matched' : bet.outcome === 'win' ? 'Bet ended with a win' : bet.outcome === 'loss' ? 'Bet ended with a loss' : isExpired ? 'This bet has expired' : 'This bet is no longer available'}
            </div>
          </div>}
      </div>
    </div>;
