    ...
    <div 
      className={`backdrop-blur-lg border rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg ${
        bet.status === 'open' 
          ...
          : bet.status === 'matched' || bet.status === 'pending'
          ? 'bg-black/30 border-purple-500/30' 
          : bet.status === 'expired' 
          ? 'bg-black/20 border-red-500/30' 
          : bet.status === 'won' || bet.outcome === 'win'
          ? 'bg-black/20 border-green-500/30' 
          : bet.status === 'lost' || bet.outcome === 'loss'
          ? 'bg-black/20 border-red-500/30' 
            ...
            <div className="text-xs text-center">
              {isLoading ? (
                ...
                <>
                  ...
                  ) : bet.prediction === 'moon' || bet.prediction === 'up' ? (
                    calculateMarketCapChange() && calculateMarketCapChange()! > 0 ? (
