...
import { X, Settings, Info, Gamepad, Clock, ArrowUpRight, ArrowDownLeft, Gift, Users, Link as LinkIcon } from 'lucide-react';
...
const PXBOnboarding: React.FC<PXBOnboardingProps> = ({
  ...
  useEffect(() => {
    if (userProfile) {
      setLocalPoints(userProfile.pxbPoints);
    }
  }, [userProfile]);
  ...
  const handleMintPoints = async () => {
    try {
      if (userProfile) {
        setLocalPoints(prev => prev !== null ? prev + pointAmount : pointAmount);
      }
          ...
          {userProfile ? (
            <div className="my-6 text-center">
              ...
              <div className="text-4xl font-bold text-amber-400">
                {localPoints !== null ? localPoints.toLocaleString() : userProfile.pxbPoints.toLocaleString()}
              </div>
