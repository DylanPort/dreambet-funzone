...
const OnlineUsersSidebar = ({ className }: OnlineUsersSidebarProps) => {
  const { onlineUsers, loading } = useOnlineUsers();

  return (
    <Card className={`bg-dream-background/20 border border-dream-foreground/10 p-4 ${className}`}>
      <div className="mb-4 flex items-center">
        <Users className="w-5 h-5 mr-2 text-dream-accent2" />
        ...
        <>
          ...
          <div className="space-y-2 max-h-[calc(100vh-240px)] overflow-y-auto">
            {onlineUsers.length === 0 ? (
              <div className="text-center py-6 text-dream-foreground/50">
