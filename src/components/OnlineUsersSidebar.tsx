...
const OnlineUsersSidebar = ({
  ...
  return (
    <Card className={`bg-dream-background/30 border border-dream-foreground/10 rounded-lg p-4 ${className}`}>
      <div className="flex items-center mb-4">
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-dream-accent1/20 to-dream-accent2/20 flex items-center justify-center mr-2">
          <Users className="w-4 h-4 text-dream-accent1" />
        </div>
        <h3 className="font-display font-bold text-lg">Online Users</h3>
        
        {!loading && onlineUsers && (
          <div className="flex items-center ml-auto text-xs text-dream-foreground/60">
        ...
        <div className="space-y-3">
          {onlineUsers && onlineUsers.length > 0 ? (
            onlineUsers.map(user => (
              <Link key={user.id} to={`/profile/${user.id}`} className="flex items-center hover:text-dream-accent1 transition-colors">
            ...
            <p className="text-dream-foreground/50 text-sm text-center py-3">No users online</p>
          )}
        </div>
      )}
    </Card>
