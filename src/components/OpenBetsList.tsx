...
const OpenBetsList = () => {
  ...
  return <Card className="p-6 rounded-xl backdrop-blur-sm bg-dream-background/30 border border-dream-accent1/20 space-y-4">
      ...
      <div className="space-y-4">
        ...
        return <div key={`${token.mint}-${index}`} className="flex items-center justify-between gap-4 relative z-10 p-4 bg-dream-background/40 rounded-lg border border-dream-accent1/10">
              <div className="flex flex-col space-y-2 w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
                  <div className="bg-dream-foreground/10 p-3 rounded-lg">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <a href={`https://solscan.io/token/${token.mint}`} target="_blank" rel="noopener noreferrer" className="text-xs text-dream-accent2 hover:text-dream-accent1 transition-colors flex-shrink-0">
                          <ExternalLink className="h-3.5 w-3.5" />
                        </a>
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button onClick={() => copyToClipboard(token.mint, index)} className="text-xs text-dream-accent2 hover:text-dream-accent1 transition-colors flex-shrink-0">
                                {copiedAddresses[index] ? <CheckCheck className="h-3.5 w-3.5 text-green-400" /> : <Copy className="h-3.5 w-3.5" />}
                              </button>
                            </TooltipTrigger>
                            <TooltipContent>
                  ...
                  <div className="bg-dream-foreground/10 p-3 rounded-lg">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <a href={`https://solscan.io/account/${token.traderPublicKey}`} target="_blank" rel="noopener noreferrer" className="text-xs text-dream-accent2 hover:text-dream-accent1 transition-colors flex-shrink-0 ml-1">
                        <ExternalLink className="h-3.5 w-3.5" />
                      </a>
                    </div>
                  </div>
                  ...
                  <div className="bg-dream-foreground/10 p-3 rounded-lg">
                    ...
                    <div className="text-sm font-medium">
                      {token.supply ? token.supply.toLocaleString() : 'Unknown'}
                    </div>
                  </div>
                ...
                <div className="flex justify-around py-4 mt-3">
                  <div className="relative group cursor-pointer" onClick={() => navigateToTokenDetail(token.mint)}>
      ...
      {rawTokens.length > 5 && <div className="flex justify-center mt-4">
          <Button variant="outline" onClick={() => setIsExpanded(!isExpanded)} className="text-xs px-4 py-2 flex items-center gap-2 bg-dream-background/40 hover:bg-dream-accent1/10 transition-colors">
            {isExpanded ? <>
