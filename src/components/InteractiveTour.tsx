...
const InteractiveTour = () => {
  ...
  return (
    <div className="flex justify-center items-center w-full my-12 mx-auto">
      <motion.div
        className="relative w-[400px] md:w-[550px] h-[230px] md:h-[280px] flex items-center justify-center bg-gradient-to-r from-gray-900/80 to-indigo-950/80 rounded-2xl border border-indigo-500/20 backdrop-blur-sm overflow-hidden"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
          ...
          >
            ...
            <div className="mt-4 flex justify-center space-x-2">
              ...
              {steps.map((_, index) => (
                <button
                  key={index}
                  className={`w-2.5 h-2.5 rounded-full transition-all ${currentStep === index ? 'bg-indigo-400 scale-125' : 'bg-gray-600 hover:bg-gray-500'}`}
            ...
            <div className="absolute bottom-4 right-4 flex space-x-2">
              ...
              {currentStep < steps.length - 1 && (
                <Button 
                  variant="ghost" 
                  size="sm" 
                  className="text-white hover:bg-indigo-600/30"
                  onClick={handleNextStep}
                >
                  Skip
                </Button>
        ...
        {[...Array(7)].map((_, i) => (
          <motion.div
            key={`line-${i}`}
            className="absolute w-[1px] h-[100px] bg-gradient-to-b from-indigo-500/0 via-indigo-500/50 to-indigo-500/0"
            style={{
              left: `${15 + i * 12}%`,
              top: '50%',
