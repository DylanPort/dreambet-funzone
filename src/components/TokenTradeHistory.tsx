...
const TokenTradeHistory: React.FC<TokenTradeHistoryProps> = ({
  ...
  const [publicTransactions, setPublicTransactions] = useState<PXBTransaction[]>([]);
  const [loading, setLoading] = useState(true);
  
  // Fetch user's own transactions from PXB context
  useEffect(() => {
  ...
  // Fetch public transactions from Supabase
  useEffect(() => {
    const fetchPublicTransactions = async () => {
      try {
        ...
        if (data) {
          const formattedTxs: PXBTransaction[] = data.map(tx => ({
            id: tx.id,
            ...
            buyerAddress: tx.userid,
            sellerAddress: tx.type === 'sell' ? tx.userid : undefined,
            username: tx.users?.username
          }));
          setPublicTransactions(formattedTxs);
        }
      } catch (error) {
        console.error("Error processing public token transactions:", error);
      }
    };
    
    if (tokenId) {
      fetchPublicTransactions();
      // Refresh public transactions every minute
      const interval = setInterval(fetchPublicTransactions, 60000);
      return () => clearInterval(interval);
    }
  }, [tokenId]);
  ...
  const displayTransactions = publicTransactions.length > 0 ? publicTransactions : transactions;
  
  return <div className="space-y-4 max-h-96 overflow-y-auto">
      {loading && displayTransactions.length === 0 && <div className="text-center py-8 text-dream-foreground/70">
      ...
      {!loading && displayTransactions.length === 0 ? <div className="text-center py-8 text-dream-foreground/70">
          ...
          <CardContent className="p-4 pt-0">
            <div className="space-y-3">
              {displayTransactions.map((trade, index) => <div key={`${trade.id}-${index}`} className="border-t border-dream-accent1/10 pt-3 first:border-t-0 first:pt-0">
                  ...
                  <div className="flex items-center justify-between text-xs bg-black/20 p-2 rounded">
                    <div className="flex items-center">
                      ...
                      <span className="text-dream-foreground/70">
                        {trade.username ? `${trade.username}` : (trade.type === 'buy' ? 'Buyer: ' : 'Seller: ') + 
