...
interface FuturisticTokenDisplayProps {
  tokens?: any[];
  tokenId?: string;
}

const FuturisticTokenDisplay: React.FC<FuturisticTokenDisplayProps> = ({
  tokens: propTokens,
  tokenId
}) => {
  ...
  useEffect(() => {
    if (tokenId && (!propTokens || propTokens.length === 0)) {
      const mockToken = {
        id: tokenId,
        name: "Token",
        symbol: "TKN",
      ...
      setTokens([mockToken]);
    } else if (propTokens && propTokens.length > 0) {
  ...
  useEffect(() => {
    if (tokens.length > 1) {
      intervalRef.current = setInterval(rotateToNextToken, 5000);
    }
    return () => {
