...
const TokenDetail = () => {
  ...
  const handleChartPriceUpdate = useCallback((price: number, change24h: number = 0) => {
    ...
    try {
      localStorage.setItem(`token_price_${id}`, JSON.stringify({ price, change24h, timestamp: Date.now() }));
    } catch (error) {
      ...
      try {
        ...
        if (tokenData) {
          ...
          for (let i = -30; i <= 0; i++) {
            ...
            initialData.push({ time: time.toISOString(), price: tokenData.last_trade_price || 0 });
          }
          ...
          for (let i = -30; i <= 0; i++) {
            const time = new Date(now);
            time.setMinutes(time.getMinutes() + i);
            initialData.push({ time: time.toISOString(), price: dexScreenerData?.priceUsd || 0 });
          }
          setPriceData(initialData);
  ...
  const { userProfile, bets: userPXBBets, fetchUserBets, isLoading: pxbLoading } = usePXBPoints();
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                ...
                <div className="glass-panel p-4 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-dream-foreground/70 text-sm">Current Price:</span>
                    <span className="font-bold text-lg">${formatPrice(token.currentPrice)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-dream-foreground/70 text-sm">24h Change:</span>
                    <span className={`font-medium flex items-center ${token.change24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                      {token.change24h >= 0 ? (
                  ...
                  <div className="glass-panel p-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="glass-panel border border-dream-accent1/20 p-4">
                        ...
                        <Button 
                          className="w-full bg-gradient-to-r from-dream-accent1 to-dream-accent2 hover:from-dream-accent1/90 hover:to-dream-accent2/90 transition-all"
                      ...
                      <div>
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-dream-accent2 hover:underline flex items-center text-sm justify-end"
                        >
                          <ExternalLink className="w-3 h-3 mr-1" />
                          View on DexScreener
                        </a>
                    ...
                    <div className="space-y-4 max-h-96 overflow-y-auto">
                      {tokenPXBBets.map(bet => (
                        <div key={bet.id} className="border border-dream-foreground/10 rounded-md p-4">
                          ...
                          {bet.status === 'pending' && marketCapData[bet.id] && (
