...
import { createSupabaseBet } from '../supabase/betService';
import { getWalletPublicKey, isTestMode, getRpcUrl } from './walletUtils';
import { getPredictionValue } from './betTypesUtils';
import { storeBetLocally } from './localStorageUtils';
...
export const createSolanaBet = async (
  ...
  try {
    ...
    storeBetLocally(localBet);

    // Display a toast notification for bet creation only
    toast({
...
export const acceptSolanaBet = async (
  ...
  try {
    ...
    // For local development: Check if we're in test/development mode
    const testMode = isTestMode();
    console.log(`Running in ${testMode ? 'TEST' : 'PRODUCTION'} mode for accepting bet`);
