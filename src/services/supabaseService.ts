...
export const fetchLatestBets = async (limit: number = 5, includeExpired: boolean = false) => {
  try {
    console.log("Fetching latest bets from Supabase...");
    
    // Build the query with filters based on the includeExpired parameter
    let query = supabase
      .from('bets')
      .select(`
        bet_id,
    ...
    // Only filter status if we're not including expired bets
    if (!includeExpired) {
    ...
    if (error) {
      console.error('Supabase error fetching latest bets:', error);
      throw error;
    }
    
    console.log('Raw latest bets data from Supabase:', data);
    
    if (!data || data.length === 0) {
      console.log('No latest bets found in database');
      return [];
    }
    ...
    console.log('Final transformed latest bets:', transformedBets);
    return transformedBets;
  } catch (error) {
